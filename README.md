...
> **R:Eso es porque #include copia todo el contenido completo de los headers del sistema, aumentando muchisimo la cantidad de lineas** 
...
> **R:No devuelve nada porque los comentarios en programa.i desaparecen.**
...
> **R:Comunican el numero de linea  y el archivo.  **
...
> **R: Va aumentado una variable cada vez que se llama a la funcion y dedvuelve el resultado de a + b. Las instrucciones suman con add a "llamadas" y usa mov para ir moviendo a y b a los distintos registros y asi poder sumarlos con add y devolverlos.**
...
> **R:Porque el archivo programa.o esta haciendo referencia a funciones o variables que no están en él: viven en otro .o o en una biblioteca entonces les pone la U**
...
> **R:CUADRADO(x) es una instruccion que reemplaza ese texto por otro y la funcion real es un bloque de codigo que cuando lo llamas el programa "salta" a ejecutar el codigo. La primera desaparece en preprocesamiento y la otra se convierte en instrucciones de maquina en compilacion/ensamblado. La funcion real si tiene verificacion de tipos. **
...
> **R:El tipo T se define en la seccion text (codigo ejecutable), y el tipo D se define en la seccion data, y es una variable global con valor inicial. **
...
> **R:ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7fffc0a70000)
        KERNEL32.DLL => /c/WINDOWS/System32/KERNEL32.DLL (0x7fffc0190000)    
        KERNELBASE.dll => /c/WINDOWS/System32/KERNELBASE.dll (0x7fffbe360000)
        ucrtbase.dll => /c/WINDOWS/System32/ucrtbase.dll (0x7fffbe100000)    **

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:No hubo que especificarla porque gcc no es solo un compilador, es un "driver" o controlador.**
